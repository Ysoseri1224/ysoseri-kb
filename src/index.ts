...
*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;line-height:1.6}
  main{width:min(1180px,calc(100% - 40px));margin:0 auto;padding:34px 0 70px}.back-home{display:inline-flex;align-items:center;gap:6px;margin-bottom:22px;color:var(--muted);font-size:13px;text-decoration:none}.back-home:hover{color:var(--ruby)}.masthead{display:flex;justify-content:space-between;gap:24px;align-items:flex-start;border-bottom:1px solid var(--line);padding-bottom:24px}.eyebrow{font-size:12px;letter-spacing:.16em;text-transform:uppercase;color:var(--ruby);font-weight:700}.title{margin:7px 0 4px;font-family:Georgia,"Times New Roman",serif;font-size:clamp(32px,5vw,58px);line-height:1.05;font-weight:600}.subtitle{margin:0;color:var(--muted);max-width:620px}.source{font-size:12px;color:var(--muted);text-align:right}.source a{color:var(--ruby)}
...
@media(max-width:800px){main{width:min(100% - 24px,680px);padding-top:20px}.masthead{display:block}.source{text-align:left;margin-top:14px}.layout{grid-template-columns:1fr}.detail{position:static}.stats{grid-template-columns:repeat(3,1fr)}.search{grid-template-columns:1fr}.search button{height:46px}}
  ...
  <a class="back-home" href="/" aria-label="返回主站">← 返回主站</a><header class="masthead"><div><div class="eyebrow">Private Knowledge Base</div><h1 class="title">私人知识库</h1><p class="subtitle">NIST Interatomic Potentials Repository 的可检索个人镜像。界面使用中文，原始技术内容保留英文。</p></div><div class="source">非官方镜像<br><a href="https://www.ctcms.nist.gov/potentials/" target="_blank" rel="noreferrer">查看 NIST 原站 ↗</a><div id="sync-status" class="status">正在读取同步状态…</div></div></header>
...
<nav class="panel" id="source-nav" style="margin-bottom:18px"><h2>原站导航</h2><div class="status">正在读取导航页…</div></nav>
...
async function showPage(id){const r=await fetch(apiBase+'/page/'+encodeURIComponent(id));const d=await r.json();if(!r.ok){$('#detail').innerHTML='<div class="empty">页面读取失败</div>';return}$('#detail').innerHTML='<div class="label">原站页面</div><h3>'+esc(d.page.title||d.page.path)+'</h3><div class="html source-content">'+(d.page.content_html||'<p>该页面没有可提取正文。</p>')+'</div><p class="fineprint"><a href="'+esc(d.page.url)+'" target="_blank" rel="noreferrer">查看 NIST 原页 ↗</a></p>'}
async function loadSourceNav(){const r=await fetch(apiBase+'/pages');if(!r.ok)return;const d=await r.json();$('#source-nav').innerHTML='<h2>原站导航</h2><div class="entry-list">'+(d.pages||[]).map(p=>'<div><a href="'+routeBase+'/page/'+encodeURIComponent(p.id)+'" data-page="'+esc(p.id)+'">'+esc(p.title||p.path)+'</a><span class="status"> · '+esc(p.path)+'</span></div>').join('')+'</div>';$('#source-nav').querySelectorAll('[data-page]').forEach(a=>a.addEventListener('click',e=>{e.preventDefault();showPage(a.dataset.page)}))}
...
loadStats();search();loadSourceNav();
</script></body></html>`;
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === '/api/pages') return json({ pages: (await env.DB.prepare("SELECT id, url, path, title, last_modified, fetched_at FROM source_pages WHERE path NOT LIKE '/potentials/system/%' ORDER BY path").all()).results ?? [] });
    ...
    if (path.startsWith('/api/page/')) {
      const id = normalizeId(path.slice('/api/page/'.length));
      const page = await env.DB.prepare('SELECT * FROM source_pages WHERE id = ?').bind(id).first();
      if (!page) return json({ error: 'page_not_found' }, 404);
      const chunks = await env.DB.prepare('SELECT content_html, content_text FROM source_page_chunks WHERE page_id = ? ORDER BY chunk_index').bind(id).all<any>();
      return json({ page: { ...page, content_html: (chunks.results ?? []).map((chunk) => chunk.content_html ?? '').join(''), content_text: (chunks.results ?? []).map((chunk) => chunk.content_text ?? '').join('') } });
    }
    if (path === '/search' || path === '/updates' || path.startsWith('/system/') || path.startsWith('/entry/') || path.startsWith('/page/')) return html(pageShell());
